  ...
  return (
    <div className="page-container" style={{ maxWidth: '800px', margin: '0 auto', paddingTop: '40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '24px', marginBottom: '40px' }}>
        ...
        {/* KPI: Llegadas */}
        <div style={{ background: 'var(--surface-1)', padding: '24px', borderRadius: '16px', border: '1px solid color-mix(in srgb, var(--warning) 35%, transparent)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '12px', color: 'var(--warning)' }}>
            <Icon icon={LogIn} size="3xl" />
          </div>
          <span style={{ fontSize: '3rem', fontWeight: 900, color: 'var(--text-base)', display: 'block', lineHeight: 1 }}>{arrivalsToday.length}</span>
          <span style={{ fontSize: '0.85rem', color: 'var(--warning)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', marginTop: '8px', display: 'block' }}>Llegadas Hoy</span>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{pendingArrivals} por ingresar</span>
        </div>

        {/* KPI: Salidas */}
        <div style={{ background: 'var(--surface-1)', padding: '24px', borderRadius: '16px', border: '1px solid var(--border)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '12px', color: 'var(--neutral)' }}>
            <Icon icon={LogOut} size="3xl" />
          </div>
          <span style={{ fontSize: '3rem', fontWeight: 900, color: 'var(--text-base)', display: 'block', lineHeight: 1 }}>{departuresToday.length}</span>
          <span style={{ fontSize: '0.85rem', color: 'var(--neutral)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', marginTop: '8px', display: 'block' }}>Salidas Hoy</span>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{pendingDepartures} por salir</span>
        </div>

        {/* KPI: Pendientes */}
        <div style={{ background: 'var(--surface-1)', padding: '24px', borderRadius: '16px', border: '1px solid color-mix(in srgb, var(--danger) 35%, transparent)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '12px', color: 'var(--danger)' }}>
            <Icon icon={AlertCircle} size="3xl" />
          </div>
          <span style={{ fontSize: '3rem', fontWeight: 900, color: 'var(--text-base)', display: 'block', lineHeight: 1 }}>{totalPending}</span>
          <span style={{ fontSize: '0.85rem', color: 'var(--danger)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', marginTop: '8px', display: 'block' }}>Tareas Pendientes</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '400px', margin: '0 auto' }}>
        <Link href="/calendario" className="btn-primary" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', padding: '16px', borderRadius: '12px', fontWeight: 600, textDecoration: 'none', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
          <Icon icon={PlusCircle} size="xl" />
        ...
        <button style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', padding: '16px', background: 'var(--success)', color: 'var(--surface-1)', borderRadius: '12px', fontWeight: 600, border: 'none', cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
