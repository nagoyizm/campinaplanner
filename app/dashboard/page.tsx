...
export default async function DashboardPage() {
  ...
  const totalGuestsCheckedIn = checkedInReservations.reduce((sum, r) => sum + r.adults + r.children, 0)
  
  // Total occupied rooms today is the sum of rooms in the checked_in reservations
  const occupiedRoomsToday = checkedInReservations.reduce((sum, r) => sum + r.rooms.length, 0)
  const occupancyPercent = totalActiveRooms > 0 ? Math.round((occupiedRoomsToday / totalActiveRooms) * 100) : 0
        ...
        <div className={`card ${styles.kpiCard}`}>
          <div className="card-body">
            ...
            <div className={styles.kpiContent}>
              ...
              <span className={styles.kpiSubtext}>
                En {checkedInCount} cabaña{checkedInCount !== 1 ? 's' : ''}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className={`card ${styles.kpiCard}`}>
          <div className="card-body">
            ...
            <div className={styles.kpiContent}>
              ...
              <span className={styles.kpiSubtext}>
                {occupiedRoomsToday} de {totalActiveRooms} unidades
              </span>
            </div>
          </div>
        </div>
